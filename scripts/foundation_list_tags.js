...
hexo.extend.helper.register('foundation_list_tags', function(tags, options) {
    ...
    if (style === 'list') {
        ...
        tags.forEach(function(tag) {
            ...
            if (showCount) {
                result += ' <span class="' + className + '-list-count badge">' + tag.length + '</span>';
            }
        ...
        tags.forEach(function(tag, i) {
            ...
            result += '<a class="' + className + '-link label" href="' + self.url_for(tag.path) + '">';
            result += transform ? transform(tag.name) : tag.name;
